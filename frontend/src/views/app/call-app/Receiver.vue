<template>
  ...
  >
    ...
    >
      ...
      <h1 class="mt-5 text-black-50 mb-5">
        Incoming call from <strong>{{ displayUser.name }}</strong>
      </h1>
      <button
        type="button"
        @click="answerCall"
        class="btn btn-lg btn-success rounded-pill px-5 me-3"
      >
    ...
    <div v-show="callingStatus === 'connected'">
      ...
      <div
        class="call-controls text-center align-self-center p-3 bg-primary bg-opacity-10"
      >
        <mic-button :click-callback="toggleLocalAudio"></mic-button>
        <video-button :click-callback="toggleLocalVideo"></video-button>
        <button
          @click="rejectCall"
          class="btn btn-lg btn-danger rounded-circle mx-1"
        >
          <i class="fa-solid fa-phone" style="transform: rotate(133deg)"></i>
        </button>
      </div>
  ...
  methods: {
    initializePeer() {
      ...
      this.peer.on("open", (id) => {
        this.peer_id = id;
        console.log("My peer id", this.peer_id);
    ...
    initializeWebSocket(peer_id) {
      this.socket = new WebSocket(
        `${import.meta.env.VITE_WS_ENDPOINT}ws/message/${peer_id}/`
      );
